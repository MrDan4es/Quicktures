        ...
        checkImage($('#url-add-form').val(), () => {
            ...
            $.ajax({
                ...
                success: response => {
                    ...
                    <div class="col" id="block-image-${response.id}">
                        <div class="card image-card ratio ratio-1x1" id="image-card-${response.id}">
                            <img src="${response.url}" class="img-block" id="${response.id}" alt="...">
                        
                            <a href="javascript:void(0);" class="btn p-0 btn-outline-primary btn-inform" id="${response.id}" data-title="${response.title}" data-date="${response.date_create}">
                            ...
                            <a href="javascript:void(0);" class="btn p-0 btn-outline-danger btn-remove" id="${response.id}">
                    ...
                    if (!motionMatchMedia.matches && !(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))) {
                        let item = document.querySelector(`#image-card-${response.id}`)
